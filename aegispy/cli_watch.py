...
from aegispy.watcher import FileWatcher, WatchEvent, WatchEventType
...
@click.option(
    "--output", "-o",
    type=click.Path(),
    help="Write events to file instead of stdout"
)
def watch(paths: tuple[str], interval: float, recursive: bool, no_recursive: bool,
          follow_symlinks: bool, ignore: tuple[str], output: str | None) -> None:
    ...
    def event_handler(event: WatchEvent) -> None:
        ...
        if output_file:
            ...
            output_file.write_text(line, append=True)
        else:
            click.echo(
                f"{event.timestamp:.3f} {event.event_type.value:12} {event.path}",
                err=True
            )
...
# Import time for the watch command
import time
