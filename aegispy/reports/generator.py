...
class ReportGenerator:
    ...
    def _generate_html(self, data: ReportData) -> str:
        ...
        # Generate execution results table
        results_rows = ""
        for result in data.execution_results:
            cells = "".join(
                f"<td>{value}</td>" for value in result.values()
            )
...
<body>
    <div class="container">
        ...
        <h2>Metadata</h2>
        <div class="metadata">
            {''.join(f'<div class="metadata-item"><strong>{k}</strong>: {v}</div>' for k, v in data.metadata.items())}
        </div>
        ...
        <table>
            <thead>
                <tr>{''.join(f'<th>{k}</th>' for k in data.execution_results[0].keys()) if data.execution_results else ''}</tr>
            </thead>
            <tbody>
                {results_rows}
            </tbody>
        </table>
        """ if data.execution_results else ""}
        
        {f"""
        <h2>Security Analysis</h2>
        <div class="section">
            {''.join(f'<p><strong>{k}</strong>: {v}</p>' for k, v in data.security_analysis.items())}
        </div>
        """ if data.security_analysis else ""}
    </div>
</body>
</html>
"""
