...
@click.option("--file", "-f", type=click.Path(exists=True), help="Path to Python file to execute")
@click.option(
    "--memory", "-m", type=int, default=None, help="Maximum memory in MB (default: from config)"
)
@click.option(
    "--timeout",
    "-t",
...
@click.option(
    "--no-security-check", is_flag=True, help="Disable security analysis (NOT RECOMMENDED)"
)
@click.pass_context
def run(
    ctx: click.Context,
...
@click.option("--output", "-o", type=click.Path(), help="Output file for report (default: stdout)")
