...
class SecurityConfig:
    ...
    def __init__(
        ...
        logger.info(
            "SecurityConfig initialized with memory_limit=%dMB, cpu_time=%ds",
            max_memory_mb,
            max_cpu_time,
        )
...
class DangerousPatternDetector:
    ...
    def __init__(self) -> None:
        ...
        self._compiled_patterns = [__import__("re").compile(p) for p in self.DANGEROUS_PATTERNS]
        logger.debug(
            "DangerousPatternDetector initialized with %d patterns", len(self._compiled_patterns)
        )
