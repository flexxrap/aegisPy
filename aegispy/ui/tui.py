...
def main() -> None:
    ...
    while True:
        try:
            ...
            if choice == "1":
                execute_code(sandbox, detector)
            elif choice == "2":
                analyze_code(detector)
            elif choice == "3":
                show_stats(sandbox)
            elif choice == "4":
...
def execute_code(sandbox: SecureSandbox, detector: DangerousPatternDetector) -> None:
    ...
    try:
        ...
        if result.stderr:
            console.print(
                Panel(result.stderr, title="[yellow]Errors[/yellow]", border_style="yellow")
            )
...
def analyze_code(detector: DangerousPatternDetector) -> None:
    ...
    risk_color = (
        "green" if analysis["is_safe"] else "yellow" if analysis["risk_score"] < 15 else "red"
    )
    ...
    safe_text = "Yes" if analysis["is_safe"] else "No"
    safe_color = "green" if analysis["is_safe"] else "red"
    report.add_row("Safe:", f"[{safe_color}]{safe_text}[/{safe_color}]")
